...
class UI:
    def __init__(self, master):
        self.master = master
        master.title("Лабораторная работа №4. ИУ7-83Б Авдейкина")
    ...
    def create_pfe_widgets(self):
        ...
        self.pfe_run_button_ockp = tk.Button(self.pfe_frame, text="Запуск ОЦКП", command=self.run_ockp, font=self.font)
        self.pfe_run_button_ockp.grid(row=7, column=0, columnspan=3, sticky='ew', pady=5, padx=5)

        # Обновляем список виджетов
        self.pfe_widgets.extend([self.pfe_run_button_ockp])

    def run_ockp(self):
        """Запуск эксперимента по ОЦКП"""
        ...
        # Создаем объект PFE и запускаем ОЦКП
        pfe = Pfe(lambda1_min, lambda1_max, lambda2_min, lambda2_max,
                  mu1_min, mu1_max, mu2_min, mu2_max, requests)
        pfe.run_ockp()

        # Выводим результаты в консоль и файл
        self.process_ockp_results(pfe)

    def process_ockp_results(self, pfe):
        """Обработка и вывод результатов ОЦКП"""
        RND = 6  # Количество знаков после запятой
        EPS = 1e-9  # Порог для округления до нуля

        # Формируем заголовки для таблицы
        headers = ('№ x0 x1(lambda1) x2(lambda2) x3(mu1) x4=x1x2x3 x1x2 x2x3 x1x3 x1^2 x2^2 x3^2 ' +
                   't1 t1^ |t1-t1^| (t1-t1^)^2 t2 t2^ |t2-t2^| (t2-t2^)^2').split()

        # Сохраняем результаты в файл
        with open('ockp_results.txt', 'w') as f:
            f.write(' '.join(headers) + '\n')
            f.write('\n'.join(' '.join(str(round(cell, RND)) for cell in row)
                              for row in pfe.ockp_result_matrix))

        # Формируем уравнения регрессии
        equation_terms = ['x0', 'x1', 'x2', 'x3', 'x4', 'x1x2', 'x2x3', 'x1x3', 'x1^2', 'x2^2', 'x3^2']
        equation_coefs = ['b0', 'b1', 'b2', 'b3', 'b4', 'b12', 'b23', 'b13', 'b11', 'b22', 'b33']

        pfe.calculate_b_coefs_natural_ockp()

        # Для первого отклика (y1)
        s_1_equation = ['Y1^ ='] + [
            f"{'-' if b < 0 else '+'} {abs(round(b, RND))}*{term}"
            for term, b in zip(equation_terms, pfe.b_coefs_vector_1_ockp)
        ]

        # Для второго отклика (y2)
        s_2_equation = ['Y2^ ='] + [
            f"{'-' if b < 0 else '+'} {abs(round(b, RND))}*{term}"
            for term, b in zip(equation_terms, pfe.b_coefs_vector_2_ockp)
        ]

        # Для первого отклика (y1) натуральное
        s_1_equation_nat = ['Y1^ ='] + [
            f"{'-' if b < 0 else '+'} {abs(round(b, RND))}*{term}"
            for term, b in zip(equation_terms, pfe.b_coefs_vector_1_natural_ockp)
        ]

        # Для второго отклика (y2) натуральное
        s_2_equation_nat = ['Y2^ ='] + [
            f"{'-' if b < 0 else '+'} {abs(round(b, RND))}*{term}"
            for term, b in zip(equation_terms, pfe.b_coefs_vector_2_natural_ockp)
        ]

        # Выводим уравнения в консоль
        print('\nРезультаты ОЦКП')
        print('\nНормированные уравнения:')
        print(' '.join(s_1_equation))
        print(' '.join(s_2_equation))

        print('\nНатуральные уравнения:')
        print(' '.join(s_1_equation_nat))
        print(' '.join(s_2_equation_nat))


        # Выводим остаточные суммы
        print(f'\nОстаточная сумма для Y1: {pfe.sum_r_nonlinear_1_ockp:.5g}')
        print(f'Остаточная сумма для Y2: {pfe.sum_r_nonlinear_2_ockp:.5g}')

        # Создаем PrettyTable для коэффициентов
        coeff_table = PrettyTable()
        coeff_table.field_names = ['Коэффициент', 'Y1', 'Y2']
        for i, term in enumerate(equation_coefs):
            coeff_table.add_row([
                term,
                round(pfe.b_coefs_vector_1_ockp[i], RND) if abs(
                    pfe.b_coefs_vector_1_ockp[i]) > EPS else f'{pfe.b_coefs_vector_1_ockp[i]:.5g}',
                round(pfe.b_coefs_vector_2_ockp[i], RND) if abs(
                    pfe.b_coefs_vector_2_ockp[i]) > EPS else f'{pfe.b_coefs_vector_2_ockp[i]:.5g}'
            ])
        print('\nКоэффициенты регрессии:')
        print(coeff_table)

        # Создаем PrettyTable для плана эксперимента
        plan_table = PrettyTable(headers)
        n = 1
        for row in pfe.ockp_result_matrix:
            plan_table.add_row([n] + [round(val, RND) if abs(val) > EPS else f'{val:.5g}' for val in row])
            n += 1
        print('\nМатрица плана и результатов:')
        print('Ядро: ДФЭ, №№1-8\n"Звездные" точки: №№9-14\nЦентральная точка: №15')
        print(plan_table)

        # Показываем результаты в GUI
        result_text = (f"Уравнение для Y1:\n{' '.join(s_1_equation)}\n\n" +
                       f"Уравнение для Y2:\n{' '.join(s_2_equation)}\n\n" +
                       f"Остаточная сумма Y1: {pfe.sum_r_nonlinear_1_ockp:.5g}\n" +
                       f"Остаточная сумма Y2: {pfe.sum_r_nonlinear_2_ockp:.5g}")

        self.show_result_window("Результаты ОЦКП", result_text)

    def show_result_window(self, title, content):
        """Показывает окно с результатами"""
        result_window = tk.Toplevel(self.master)
        result_window.title(title)

        text = tk.Text(result_window, wrap=tk.WORD, font=self.font)
        text.insert(tk.END, content)
        text.config(state=tk.DISABLED)
        text.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)

        scrollbar = ttk.Scrollbar(result_window, command=text.yview)
